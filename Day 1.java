...
public class Main {
	...
	public static void main(String[] args) {
		...
		boolean part2Finished = false;
		int part1Answer = 0;
		int part2Answer = 9999;
		int loopCount = 0;
		ArrayList<Integer> frequencyMemory = new ArrayList<Integer>();
//		String theText = "-6+3+8+5-6";
		...
		String textWithSpaces = addSpaces(theText);
		
		//This is disgusting but my split will remember the first space and make the first array index blank, breaking everything to fuck
		textWithSpaces = textWithSpaces.substring(1);
		
		String[] textAsArray = splitIntoArray(textWithSpaces);
		
			while(!part2Finished) {
				
				//loop thought the whole array and either add or subtract from the number for each index
				for (int i = 0; i < textAsArray.length; i++) {
					if (textAsArray[i].substring(0, 1).equals("-")) {
						frequencyCounter -= Integer.parseInt(textAsArray[i].substring(1));
					} else {
						frequencyCounter += Integer.parseInt(textAsArray[i].substring(1));
					}


					if (arrayIncludes(frequencyMemory, frequencyCounter) && !part2Finished) {
						part2Answer = frequencyCounter;
						part2Finished = true;
					}
					frequencyMemory.add(frequencyCounter);
				}
				
				//if this is the first iteration, save the answer to part 1 an continue to get the answer for part 2
				if (!part1Finished) {
					part1Answer = frequencyCounter;
					part1Finished = true;
				}	
	...
	public static boolean arrayIncludes(ArrayList<Integer> theArray, int theValue) {
		boolean returnBool = false;
		if (theArray.contains(theValue)) {
			returnBool = true;
		}
		return returnBool;
	}
	
	public static String[] splitIntoArray(String text) {
